...
expect(clientSource:find('local function isWeaponReloadTransitionActive', 1, true) ~= nil, 'transicao de recarga nao protege a hotbar')
expect(clientSource:find('if isWeaponReloadTransitionActive(true) then', 1, true) ~= nil, 'hotbar nao bloqueia guardar a arma durante a recarga')
expect(clientSource:find("Aguarde a recarga terminar antes de guardar ou trocar a arma.", 1, true) == nil, 'bloqueio da hotbar ainda gera notificacao desnecessaria')
expect(clientSource:find('local notifyInventoryWeapon', 1, true) ~= nil and clientSource:find('notifyInventoryWeapon = function', 1, true) ~= nil, 'notificacao da hotbar depende de global inexistente')
